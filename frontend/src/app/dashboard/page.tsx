...
import {
    ...
    Flame, ExternalLink, Zap, Calendar, ChevronDown, X
} from 'lucide-react';
...
function getPresetRange(key: PresetKey): DateRange | null {
    ...
    switch (key) {
        case 'today':
            return { start: tod, end: endOfDay(now) };
        case 'yesterday': {
            const y = new Date(tod); y.setDate(y.getDate() - 1);
        ...
        case 'last_week': {
            const lws = new Date(tod);
            lws.setDate(lws.getDate() - lws.getDay() - 7);
            const lwe = new Date(lws); lwe.setDate(lwe.getDate() + 6);
            return { start: lws, end: endOfDay(lwe) };
        }
        case 'month_to_date': {
            const m = new Date(tod); m.setDate(1);
            return { start: m, end: endOfDay(now) };
        }
        case 'last_month': {
            const lms = new Date(tod.getFullYear(), tod.getMonth() - 1, 1);
            const lme = new Date(tod.getFullYear(), tod.getMonth(), 0);
            return { start: lms, end: endOfDay(lme) };
        }
        case 'year_to_date': {
            const y = new Date(tod); y.setMonth(0); y.setDate(1);
            return { start: y, end: endOfDay(now) };
        }
        case 'last_year': {
            const lys = new Date(tod.getFullYear() - 1, 0, 1);
            const lye = new Date(tod.getFullYear() - 1, 11, 31);
            return { start: lys, end: endOfDay(lye) };
        }
        default:
            return null;
    }
...
function DateRangePicker({ selected, customRange, onChange }: DateRangePickerProps) {
    ...
    return (
        <div ref={ref} className="relative">
            ...
            <button
                onClick={() => setOpen(v => !v)}
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-card text-sm font-semibold text-foreground shadow-sm hover:shadow-md hover:border-indigo-300 dark:hover:border-indigo-600 transition-all duration-200 group"
            >
                        ...
                        {PRESETS.filter(p => p.key !== 'custom').map(preset => (
                            <button
                                ...
                                className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-all duration-150
                                    ${selected === preset.key && selected !== 'custom'
...
export default function DashboardPage() {
    ...
    const fetchStats = async (p: PresetKey, cr: DateRange | null) => {
        setLoading(true);
        try {
            let res;
            if (p === 'today') {
                res = await api.get('/reports/stats/today');
            } else {
                const range = p === 'custom' ? cr : getPresetRange(p);
                if (!range) return;
                res = await api.get('/reports/stats', {
                    params: {
    ...
    // Initial load — Today
    useEffect(() => { fetchStats('today', null); }, []);
    ...
    // Section heading based on preset
    const sectionLabel: Record<PresetKey, string> = {
        today:         "Today's Pulse",
